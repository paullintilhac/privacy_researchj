...
def main(argv):
    ...
    def get_loss(model, xbatch, ybatch, shift, reflect=True, stride=1):
        #print("xbatch lengths in get_loss: " + str(len(xbatch)) +", " + str(len(xbatch[0])) + ", " + str(len(xbatch[0][0])) + ", " + str(len(xbatch[0][0][0])))
        outs = []
        #thing = [xbatch, xbatch[:,:,::-1,:]]
        #print("thing lengths in get_loss: " + str(len(thing)) +", " + str(len(thing[0])) + ", " + str(len(thing[0][0])) + ", " + str(len(thing[0][0][0]))+", " + str(len(thing[0][0][0][0])))
        #print("(reflect+1): " + str(reflect+1))
        #thing = thing[:reflect+1]
        #print("thing lengths after reflect in get_loss: " + str(len(thing)) +", " + str(len(thing[0])) + ", " + str(len(thing[0][0])) + ", " + str(len(thing[0][0][0]))+", " + str(len(thing[0][0][0][0])))
        for aug in [xbatch, xbatch[:,:,::-1,:]][:reflect+1]:
            #print("aug lengths in get_loss loop: " + str(len(aug)) +", " + str(len(aug[0])) + ", " + str(len(aug[0][0])) + ", " + str(len(aug[0][0][0])))

            aug_pad = tf.pad(aug, [[0] * 2, [shift] * 2, [shift] * 2, [0] * 2], mode='REFLECT').numpy()
            #print("aug pad shape: " + str(aug_pad.shape))
    ...
    def features(model, xbatch, ybatch):
        print("xbatch lengths: " + str(len(xbatch)) +", " + str(len(xbatch[0])) + ", " + str(len(xbatch[0][0])) + ", " + str(len(xbatch[0][0][0])))
    ...
    for path in sorted(os.listdir(os.path.join(FLAGS.logdir))):
        ...
        print("before cache load")
        model = cache_load(arch)()
        print("after cache load")
        logdir = os.path.join(FLAGS.logdir, path)
        print("before checkpoint")
        checkpoint = objax.io.Checkpoint(logdir, keep_ckpts=16, makedir=True)
        ...
        for epoch in range(first,max_epoch+1):
            ...
            stats = []
            print("about to calc stats")
            for i in range(0,len(xs_all),N):
                print("i: " + str(i))
                stats.extend(features(model, xs_all[i:i+N],
                                      ys_all[i:i+N]))
                if i>0:
                    print("stats len: " + str(len(stats))+", len(stats[0]: " + str(len(stats[0])) + ", len(stats[0][0]: "+ str(len(stats[0][0])))
            stats=np.array(stats)[:,None,:,:]
            #print("final stats len: " + str(len(stats))+", len(stats[0]: " + str(len(stats[0])) + ", len(stats[0][0]: "+ str(len(stats[0][0])))
