...
def load_one(base):
    ...
    for f in os.listdir(root):
        print("predictions path: " + str(os.path.join(root,f)))
        try:
            opredictions = np.load(os.path.join(root,f))
        except:
            print("Fail")
            continue
        #print("opreds lens: " + str(len(opredictions))+", "+str(len(opredictions[0])) + ", " + str(len(opredictions[0][0])) + ", " + str(len(opredictions[0][0][0])))
        ## Be exceptionally careful.
        ## Numerically stable everything, as described in the paper.
        predictions = opredictions - np.max(opredictions, axis=3, keepdims=True)
        thing = np.max(opredictions,axis = 3,keepdims=True)
        #print("thin lens: " + str(len(thing)) + ", " + str(len(thing[0])) + ", " + str(len(thing[0][0])) + ", " + str(len(thing[0][0][0])))
